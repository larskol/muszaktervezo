<div class="card no-bgcolor card-border">
        <div class="card-body">
            ...
            <div class="row">
                ...
                <label for="department" class="col-12 col-form-label col-form-label-sm">
                    <?= lang("Icons.iconFormDepartment") ?> <?= lang("Form.formDepartment") ?>
                </label>
                <div class="col-lg-3 col-md-8 form-group">
                   <select name="department" id="department-selector" class="form-control form-control-sm input-style">
                    <option value=""><?= lang("Site.siteChoose") ?></option>
                    <?php foreach ($departments as $key => $value): ?>
                        <option value="<?= $value->id ?>" <?= isset($selectedDepartment) && $selectedDepartment == $value->id ? "selected" : "" ?>><?= $value->name ?></option>
                    <?php endforeach; ?>
                   </select>
                </div>
                <?php else: ?>
                    <div class="col-12"><?= lang("Form.formNoDepartmentsAddedYet") ?></div>
                <?php endif; ?>
            </div>
            <div class="row">
