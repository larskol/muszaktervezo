<div class="card no-bgcolor card-border">
        <div class="card-body">
            ...
            <?= form_open('', ['id' => 'knowledge-level-form']) ?>
            ...
            <div class="row form-group">
                <label for="name" class="col-lg-2 col-md-4 col-form-label col-form-label-sm">
                    <?= lang("Icons.iconFormExperience") ?> <?= lang("Form.formExperienceYear") ?>
                </label>
                <div class="col-lg-3 col-md-8">
                    <input type="text" name="experience" class="form-control form-control-sm input-style" placeholder="<?= lang("Form.formExperienceYear") ?>" value="<?= isset($item, $item->experience) ? $item->experience : '' ?>">
                    <div class="invalid-feedback"></div>
                </div>
            </div>
            <div class="row form-group">
...
<script>
    $('form#knowledge-level-form').find('input,select,textarea').on('input', function(e){
        $(this).removeClass('is-invalid');
    });
    postFormData('/knowledge-levels/save', '#knowledge-level-form', '#form-save-button', <?= isset($item) ? "'/knowledge-levels'" : "''" ?>);
</script>